...
module PostsHelper
  ...
  def show_metadata(post)
    content_tag(:p, post.published_at.strftime('<span class="month">%b</span> <span class="day">%d</span> <span class="year">&rsquo;%y</span>'), :class => 'date') + content_tag(:p, "Filed in #{post.categories.collect{|cat| link_to cat.name.downcase, blog_category_path(cat)}.join(', ')}", :class => 'metadata')
  end
