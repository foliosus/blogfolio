...
module PostsHelper
  ...
  def show_metadata(post)
    content_tag(:p, (post.published_at? ? post.published_at.strftime('<span class="month">%b</span> <span class="day">%d</span> <span class="year">&rsquo;%y</span>') : '<span class="month">DRAFT</span> <span class="day">!!</span> <span class="year">!?!</span>'), :class => 'date') + content_tag(:p, "Filed in #{post.categories.collect{|cat| link_to cat.name.downcase, blog_category_path(cat)}.join(', ')}", :class => 'metadata')
  end
  ...
  def single_post(post, options = {})
    ...
    body = simple_format(auto_link(sanitize(body.to_html, :tags => %w(pre ol ul blockquote h1 h2 h3 h4 div p object param embed), :attributes => %w(allowscriptaccess allowfullscreen alt class height href name src title type width value))))
    body = Hpricot(body)
