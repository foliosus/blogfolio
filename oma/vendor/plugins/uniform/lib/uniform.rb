...
module Reed #:nodoc:
  module UniForm
    ...
    class UniFormBuilder < ActionView::Helpers::FormBuilder
      ...
      (field_helpers - %w(hidden_field check_box radio_button apply_form_for_options!) + %w(select date_select datetime_select time_select)).each do |selector|
        src = <<-END_SRC
          ...
          def #{selector}(field,#{' choices,' if selector == 'select'} options = {})
            ...
            # Set row & column options if necessary
            if '#{selector}' == 'text_area'
              options.reverse_merge!(:rows => 20, :cols => 60)
            end
      ...
      # Output a grouped set of fields, such as a month/day/year input set, or a set of radio buttons or check boxes.  This should be used where a single label is needed for multiple <code>input</code> elements. Proper usage is:
      #
      #   <%- f.multiple_field_group :label => 'My Label' do -%>
      #     <%= f.radio_button :attribute1 %>
      #     <%= f.radio_button :attribute2 %>
      #   <%- end -%>
      def multiple_field_group(options = {}, &block)
        first = options.delete(:first)
        the_class = options.delete(:class)
        @template.concat(start_uniform_wrapper(:first => first, :class => the_class), block.binding)
        @template.concat(label_wrapper(options[:label].underscore.gsub(/ /,'_'), options), block.binding) if options[:label]
        @template.concat(hint_wrapper(options[:hint]), block.binding) if options[:hint]
        @wrapper = false
        content = @template.capture(&block)
        @template.concat(content, block.binding)
        @wrapper = true
        @template.concat(end_uniform_wrapper, block.binding)
      end
      
      # Output a fieldset, complete with legend. Proper usage is
      #
      #   <%- f.fieldset "My legend" do -%>
    ...
    def uni_form_for(name, object = nil, options = {}, &proc)
      ...
      form_for(name,
                object,
                options.merge(:builder => UniFormBuilder, :html => options[:html].merge(:class => 'uniForm ')),
                &proc)
    end
    
    # Create a wrapper function for our custom UniForm for use with AJAX-based forms
    def remote_uni_form_for(name, object = nil, options = {}, &proc)
      concat(content_tag(:p, '<em>*</em> indicates a required field', :class => 'form_required') + "\n",proc.binding)
      options[:html] ||= {}
